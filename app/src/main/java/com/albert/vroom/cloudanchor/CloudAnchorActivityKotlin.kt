...
import com.albert.vroom.cloudanchor.CloudAnchorManagerKotlin.CloudAnchorHostListener
import com.albert.vroom.cloudanchor.CloudAnchorManagerKotlin.CloudAnchorResolveListener
import com.albert.vroom.cloudanchor.FirebaseManager.RoomCodeListener
...
import com.albert.vroom.common.helpers.*
 ...
 * anchors.
 */
class CloudAnchorActivityKotlin : AppCompatActivity(), GLSurfaceView.Renderer, NoticeDialogListener {
    ...
    private val cloudManager = CloudAnchorManagerKotlin()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val context = this
        surfaceView!!.apply {
            setOnTouchListener(View.OnTouchListener { v: View?, event: MotionEvent? -> gestureDetector!!.onTouchEvent(event) })
        ...
        host_button.setOnClickListener(View.OnClickListener { onHostButtonPress() })
        resolve_button.setOnClickListener(View.OnClickListener { onResolveButtonPress() })
    ...
    override fun onDrawFrame(gl: GL10) {
        ...
        try {
            ...
            // move this stuff out, maybe look at firebase manager
            val database = Firebase.database
            val myRef = database.getReference("pose_data")
            val pose = camera.displayOrientedPose
            myRef.child("translation").setValue(pose.translation.toList())
            myRef.child("rotation").setValue(pose.rotationQuaternion.toList())
    ...
    private fun onHostButtonPress() {
        ...
        if (!sharedPreferences!!.getBoolean(ALLOW_SHARE_IMAGES_KEY, false)) {
            showNoticeDialog(HostResolveListener { onPrivacyAcceptedForHost() })
        } else {
    ...
    private fun onResolveButtonPress() {
        ...
        if (!sharedPreferences!!.getBoolean(ALLOW_SHARE_IMAGES_KEY, false)) {
            showNoticeDialog(HostResolveListener { onPrivacyAcceptedForResolve() })
        } else {
    ...
    private fun onPrivacyAcceptedForResolve() {
        ResolveDialogFragment().apply {
            setOkListener { roomCode: Long -> onRoomCodeEntered(roomCode) }
            show(supportFragmentManager, "ResolveDialog")
        }
    }
     ...
     */
    private inner class RoomCodeAndCloudAnchorIdListener : CloudAnchorHostListener, RoomCodeListener {
        ...
        override fun onNewRoomCode(newRoomCode: Long) {
            ...
            snackbarHelper.showMessageWithDismiss(
                    this@CloudAnchorActivityKotlin, getString(R.string.snackbar_room_code_available))
            checkAndMaybeShare()
            synchronized(singleTapLock) {
        ...
        override fun onError(error: DatabaseError) {
            ...
            snackbarHelper.showError(
                    this@CloudAnchorActivityKotlin, getString(R.string.snackbar_firebase_error))
        }
        ...
        override fun onCloudTaskComplete(anchor: Anchor?) {
            ...
            if (cloudState.isError) {
                ...
                snackbarHelper.showMessageWithDismiss(
                        this@CloudAnchorActivityKotlin, getString(R.string.snackbar_host_error, cloudState))
                return
            }
            Preconditions.checkState(
        ...
        private fun checkAndMaybeShare() {
            ...
            snackbarHelper.showMessageWithDismiss(
                    this@CloudAnchorActivityKotlin, getString(R.string.snackbar_cloud_id_shared))
        }
    }

    private inner class CloudAnchorResolveStateListener internal constructor(private val roomCode: Long) : CloudAnchorResolveListener {
        override fun onCloudTaskComplete(anchor: Anchor?) {
            ...
            if (cloudState.isError) {
                Log.w(
                        ...
                        "The anchor in room "
                                ...
                                + cloudState)
                snackbarHelper.showMessageWithDismiss(
                        this@CloudAnchorActivityKotlin, getString(R.string.snackbar_resolve_error, cloudState))
                return
            }
            snackbarHelper.showMessageWithDismiss(
                    this@CloudAnchorActivityKotlin, getString(R.string.snackbar_resolve_success))
            setNewAnchor(anchor)
        }
        ...
        override fun onShowResolveMessage() {
            ...
            snackbarHelper.showMessageWithDismiss(
                    this@CloudAnchorActivityKotlin, getString(R.string.snackbar_resolve_no_result_yet))
        }
    ...
    companion object {
        private val TAG = CloudAnchorActivityKotlin::class.java.simpleName
        ...
        private fun shouldCreateAnchorWithHit(hit: HitResult): Boolean = hit.trackable.run {
            when (this) {
                // Check if the hit was within the plane's polygon.
                is Plane -> isPoseInPolygon(hit.hitPose)
                // Check if the hit was against an oriented point.
                is Point -> orientationMode == Point.OrientationMode.ESTIMATED_SURFACE_NORMAL
                else -> false
            }
        }
    }
